    ...
    def _extract_track_path(self, roi):
        ...
        try:
            ...
            def follow_track(track_points, start):
                ...
                current = start
                visited = {start}
                last_direction = None  # 记录上一次的移动方向
                ...
                while True:
                    ...
                    if not neighbors:  # 没有下一个点了，说明到达终点
                        break
                    
                    # 如果有上一次的移动方向，优先选择相似方向
                    if last_direction:
                        last_dx, last_dy = last_direction
                        next_point = min(neighbors, 
                                       key=lambda n: abs(n[2] - last_dx) + abs(n[3] - last_dy))
                        next_point = (next_point[0], next_point[1])
                        last_direction = (next_point[0] - current[0], next_point[1] - current[1])
                    else:
                        # 第一次移动，选择最近的点
                        next_point = min(neighbors, 
                                       key=lambda n: abs(n[2]) + abs(n[3]))[:2]
                        last_direction = (next_point[0] - current[0], next_point[1] - current[1])
...
if __name__ == "__main__":
    print("启动AFK检测自动求解器...")
    solver = AFKCheckSolver(check_interval=2)
    solver.start()
